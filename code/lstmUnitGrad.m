function [dc, dh, lstm_grad] = lstmUnitGrad(model, lstm, x_t, dc, dh, l, t, srcMaxLen, zero_state, params)
  dc = dc + params.nonlinear_f_prime(lstm{l, t}.f_c_t).*lstm{l, t}.o_gate.*dh;

  di = params.nonlinear_gate_f_prime(lstm{l, t}.i_gate).*lstm{l, t}.a_signal.*dc;
  if t>1
    df = params.nonlinear_gate_f_prime(lstm{l, t}.f_gate).*lstm{l, t-1}.c_t.*dc;
  else
    df = zero_state;
%     if params.isGPU
...
%     end
  ...
  do = params.nonlinear_gate_f_prime(lstm{l, t}.o_gate).*lstm{l, t}.f_c_t .* dh;
  ...
  if (t>=srcMaxLen) % grad tgt
    ...
    if t==1
      ...
      lstm_grad.W_src = [di; df; do; da]*[x_t; lstm{l, t-1}.h_t]';
    end
  end

  lstm_grad.dx = W(:, 1:params.lstmSize)'*[di; df; do; da];
  dc = lstm{l, t}.f_gate.*dc;
