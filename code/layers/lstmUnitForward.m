function [lstmState] = lstmUnitForward(W, x_t, h_t_1, c_t_1, params, isTest, isFeedInput) %rnnFlags)
% LSTM unit
% Input:
%   W: parameter
%   x_t: current input
...
% Thang Luong @ 2014, 2015, <lmthang@stanford.edu>
  ...
  %% dropout
  if params.dropout<1 && isTest == 0 % rnnFlags.test==0
    if ~params.isGradCheck
      ...
      if isFeedInput % rnnFlags.feedInput
...
%     x_t(1:params.lstmSize,:) = x_t(1:params.lstmSize,:).*dropoutMask;
  ...
  lstmState.h_t = h_t;
  lstmState.c_t = c_t;
  if isTest == 0 %(rnnFlags.test==0) % store intermediate results
