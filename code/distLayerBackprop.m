function [grad_mu] = distLayerBackprop(grad_distWeights, h2sInfo, params)
  ...
  if params.isGPU
    grad_mu = arrayfun(@gradMu, grad_distWeights(h2sInfo.linearIdSub), h2sInfo.alignWeights(h2sInfo.linearIdSub), h2sInfo.scaleX)/(2*params.distSigma);
  else
    grad_mu = grad_distWeights(h2sInfo.linearIdSub).*h2sInfo.distWeights(h2sInfo.linearIdSub).*h2sInfo.scaleX/(2*params.distSigma);
  end
