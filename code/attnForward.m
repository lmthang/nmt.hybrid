function [attnHidVecs, attn_h_concat, alignWeights, alignScores, attnInput] = attnForward(tgt_h_t, model, params, curData)
%%%
%
...
%
%%%
  attnInput = [tgt_h_t; curData.srcLens];
  ...
  alignWeights = permute(bsxfun(@times, alignWeights, curData.mask), [3, 2, 1]);
  ...
  % sum over maxSentLen
  attnVecs = squeeze(sum(bsxfun(@times, curData.srcAlignStates, alignWeights), 3)); 
  if params.assert % lstmSize x curBatchSize
    assert(size(attnVecs, 1)==params.lstmSize);
    assert(size(attnVecs, 2)==curData.curBatchSize);
  end
